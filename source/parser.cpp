...
class AST {
    public:
        ...
        void safe_increment(int val, vector<token *> tokens) {
            for (int i = 0; current_token_index < tokens.size() &&
                i < val; ++i) {
                ++current_token_index;
            }
        }

        vector<token *> get_expression_tokens(vector<token *> tokens) {
        ...
        vector<token *> get_structure_tokens(vector<token *> tokens) {
            ...
            while (current_token_index < tokens.size() && count != 0) {
                ...
                if (tokens[current_token_index]->type == O_BRACKET)
                    ++count;
                if (tokens[current_token_index]->type == C_BRACKET)
                    --count;
                new_tokens.push_back(tokens[current_token_index]);
                ++current_token_index;
            }
        ...
        void build(vector<token *> tokens) {
            ...
            while (current_token_index < tokens.size()) {
                ...
                // Detect Simple Identifier
                    ...
                    // Create a new Expression and a new Assignement if an Assignator is detected
...
class Assignement : public AST {
    public:
        ...
        Assignement(string var_type, string var_name, Expression *expression) {
...
class Expression : public AST {
    public:
        ...
        Expression(vector<token *> tokens) {
...
class Structure : public AST{
    public:
        ...
        Structure(string struct_type, Expression *expression, vector<token *>tokens) {
